...
async function loadData() {
    try {
        ...
        // Загружаем пользователей из localStorage (как резерв)
        ...
        // Резервные данные
        glamps = [
            { id: 1, name: "Шале у леса", price: 8900, max_guests: 2, city: "Сочи" },
            { id: 2, name: "Сфера", price: 12400, max_guests: 3, city: "Сочи" },
            { id: 3, name: "Домик у озера", price: 6800, max_guests: 4, city: "Сочи" },
            { id: 4, name: "Лодж Горный", price: 14500, max_guests: 6, city: "Красная Поляна" }
        ];
        bookings = [
            { id: 1, glamp_id: 1, user_name: "Иван Петров", check_in: "2025-04-10", check_out: "2025-04-12", guests: 2, total_price: 17800, status: "confirmed", created_at: "2025-04-01" },
            { id: 2, glamp_id: 2, user_name: "Мария Смирнова", check_in: "2025-04-15", check_out: "2025-04-17", guests: 2, total_price: 24800, status: "pending", created_at: "2025-04-02" }
        ];
    }
}

function saveData() {
    // Не сохраняем в localStorage, т.к. данные в БД
    // Можно сохранять только настройки
    ...
    try {
        ...
        if (result.success) {
            ...
            result.data.bookingsByMonth.forEach(item => {
                monthlyData[item.month - 1] = parseInt(item.count);
            });
            ...
            // Данные по популярности домиков
            const glampNames = result.data.popularGlamps.map(g => g.name);
            const glampCounts = result.data.popularGlamps.map(g => parseInt(g.bookings_count));
            ...
            popularityChart = new Chart(document.getElementById('popularityChart'), {
                ...
                data: {
                    ...
                    datasets: [{
                        data: glampCounts,
                        backgroundColor: ['#ffd700', '#28a745', '#17a2b8', '#dc3545', '#ffc107', '#6f42c1']
                    }]
                },
                options: { responsive: true, maintainAspectRatio: true }
            });
        } else {
            // Если API не работает, показываем пустые графики
            if (bookingsChart) bookingsChart.destroy();
            bookingsChart = new Chart(document.getElementById('bookingsChart'), {
                type: 'line',
                data: { labels: [], datasets: [{ label: 'Бронирования', data: [] }] },
                options: { responsive: true }
            });
            if (popularityChart) popularityChart.destroy();
            popularityChart = new Chart(document.getElementById('popularityChart'), {
                type: 'pie',
                data: { labels: ['Нет данных'], datasets: [{ data: [1] }] },
                options: { responsive: true }
            });
        }
    } catch (error) {
...
function renderGlampsTable() {
    document.getElementById('glampsTable').innerHTML = `
        ...
        <tbody>${glamps.map(g => `<tr>
            <td>${g.id}</td>
            <td><strong>${g.name}</strong></td>
            <td>${(g.price_per_night || g.price).toLocaleString()} ₽</td>
            <td>до ${g.max_guests}</td>
            <td>${g.city}</td>
            <td><button class="btn-action btn-edit" onclick="editGlamp(${g.id})"><i class="fas fa-edit"></i></button><button class="btn-action btn-delete" onclick="deleteGlamp(${g.id})"><i class="fas fa-trash"></i></button></td>
        </tr>`).join('')}</tbody></table>
    `;
}
...
function renderBookingsTable() {
    ...
    document.getElementById('bookingsTable').innerHTML = `
        ...
        <tbody>${bookings.map(b => `<tr>
            <td>${b.id}</td>
            <td><strong>${getGlampName(b.glamp_id)}</strong></td>
            ...
            <button class="btn-action btn-delete" onclick="deleteBooking(${b.id})"><i class="fas fa-trash"></i></button></td>
        </tr>`).join('')}</tbody></table>
    `;
}
...
function renderUsersTable() {
    document.getElementById('usersTable').innerHTML = `
        ...
        <tbody>${users.map(u => `<tr>
            ...
            <td>${u.email}</td>
            <td><span class="badge badge-approved">${u.role === 'admin' ? 'Админ' : 'Пользователь'}</span></td>
            <td>${u.created_at}</td>
            <td>${u.role !== 'admin' ? `<button class="btn-action btn-delete" onclick="deleteUser(${u.id})"><i class="fas fa-trash"></i></button>` : '-'}</td>
        </tr>`).join('')}</tbody></table>
    `;
}
...
function editGlamp(id) {
    ...
    if (glamp) {
        ...
        document.getElementById('glampPrice').value = glamp.price_per_night || glamp.price;
...
document.getElementById('glampForm').addEventListener('submit', (e) => {
    ...
    const glampData = {
        ...
        image_url: document.getElementById('glampImage').value || 'https://images.unsplash.com/photo-1504280390367-361c6d9f38f4?w=600'
    };
    if (id) {
